...
ICICI_MARKERS = re.compile(r"transaction remarks|withdrawal amount|transaction details|tran particulars", re.I)
...
class IciciCsvParser:
    ...
    def parse(self, file: io.BufferedIOBase, filename: str) -> ParseResult:
        ...
        date_col = _pick_column(field_map, ["transaction date", "value date", "date", "tran date", "trans date", "posting date"])
        desc_col = _pick_column(field_map, ["transaction remarks", "remarks", "description", "narration", "particulars", "transaction details", "transaction narration"])
        withdrawal_col = _pick_column(
            field_map,
            ["withdrawal amount (inr)", "withdrawal amount", "withdrawal amt.", "debit", "debit amount", "dr", "debit amt", "withdrawals"],
        )
        deposit_col = _pick_column(
            field_map,
            ["deposit amount (inr)", "deposit amount", "deposit amt.", "credit", "credit amount", "cr", "credit amt", "deposits"],
        )
        balance_col = _pick_column(field_map, ["balance (inr)", "balance", "closing balance", "running balance", "available balance"])
