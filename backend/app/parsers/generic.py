...
DATE_HEADERS = (
    ...
    "tran date", "trans date", "booking date", "entry date", "effective date",
    "trade date", "settlement date", "process date",
)
DESC_HEADERS = (
    "description", "narration", "details", "remarks", "transaction remarks",
    "particulars", "tran particulars", "transaction details", "transaction narration",
    "transaction description", "chq/ref number", "cheque details",
    "ref no / cheque no", "transaction ref no", "reference", "reference no",
    "reference number", "description/narration",
)
WITHDRAWAL_HEADERS = (
    "withdrawal", "withdrawal amt.", "withdrawal amount",
    "withdrawal amount (inr)", "debit", "debit amount", "dr",
    "debit amt", "dr amount", "dr amt", "paid out", "amount debited",
    "cheques issued / debit", "withdrawals",
)
DEPOSIT_HEADERS = (
    "deposit", "deposit amt.", "deposit amount",
    "deposit amount (inr)", "credit", "credit amount", "cr",
    "credit amt", "cr amount", "cr amt", "paid in", "amount credited",
    "cheques deposited / credit", "deposits",
)
AMOUNT_HEADERS = (
    "amount", "transaction amount", "txn amount",
    "tran amount", "trans amount", "net amount",
)
TYPE_HEADERS = ("type", "dr/cr", "debit/credit", "cr/dr", "txn type", "transaction type")
...
def _detect_columns(field_map: dict[str, str]) -> dict[str, str] | None:
    ...
    balance_col = _first_match(field_map, (
        "balance", "closing balance", "balance (inr)", "running balance",
        "available balance", "balance amount", "ledger balance",
    ))
